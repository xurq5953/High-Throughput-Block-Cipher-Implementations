#include <iostream>
#include <x86intrin.h>
//KeyExpansion

const unsigned char SBOX[256] = {
        ,
};

const unsigned char RCON[10] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};

uint32_t roundkey[44];

uint8_t userKey[16] = {0x0f, 0x15, 0x71, 0xc9,
                       0x47, 0xd9, 0xe8, 0x59,
                       0x0c, 0xb7, 0xad, 0xd6,
                       0xaf, 0x7f, 0x67, 0x98};

inline void load8to32(const uint8_t *u8, uint32_t *u32) {
    u32[0] = u8[0] | (u8[1] << 8) | (u8[2] << 16) | (u8[3] << 24);
}

inline void load32to8_inv(const uint32_t *u32, uint8_t *u8) {
    u8[0] =u32[0] >> 24;
    u8[1] = u32[0] >> 16;
    u8[2] = u32[0] >> 8;
    u8[3] = u32[0];
}
uint32_t reverse_bit(uint32_t c) {
   uint32_t mask1 = 0x55555555;
   uint32_t mask2 = 0x33333333;
   uint32_t mask3 = 0x0f0f0f0f;
   uint32_t mask4 = 0x00ff00ff;
   uint32_t mask5 = 0x0000ffff;
    c = (c & ~mask1) >> 1 | (c & mask1) << 1;
    c = (c & ~mask2) >> 2 | (c & mask2) << 2;
    c = (c & ~mask3) >> 4 | (c & mask3) << 4;
    c = (c & ~mask4) >> 8 | (c & mask4) << 8;
    c = (c & ~mask5) >> 16 | (c & mask5) << 16;
    return c;
}


inline uint32_t g(uint32_t word, int round) {
    uint8_t byte[4];
    byte[0] = (uint8_t) word;
    byte[1] = (uint8_t) (word >> 8);
    byte[2] = (uint8_t) (word >> 16);
    byte[3] = (uint8_t) (word >> 24);
    word = (uint32_t) (SBOX[byte[1]] ^ RCON[round - 1]) |
           (uint32_t) SBOX[byte[2]] << 8 |
           (uint32_t) SBOX[byte[3]] << 16 |
           (uint32_t) SBOX[byte[0]] << 24;
    return word;
}



void key_expansion() {
    load8to32(userKey, roundkey);
    load8to32(userKey + 4, roundkey + 1);
    load8to32(userKey + 8, roundkey + 2);
    load8to32(userKey + 12, roundkey + 3);
    for (int i = 4; i < 44; i += 4) {
        roundkey[i] = roundkey[i - 4] ^ g(roundkey[i - 1], i / 4);
        roundkey[i + 1] = roundkey[i - 3] ^ roundkey[i];
        roundkey[i + 2] = roundkey[i - 2] ^ roundkey[i + 1];
        roundkey[i + 3] = roundkey[i - 1] ^ roundkey[i + 2];
    }
    // for (int i = 0; i < 44; ++i) {
    //     uint8_t byte[4];
    //     // load32to8_inv(roundkey + i, byte);
    //     // load8to32(byte, roundkey + i);
    //     // roundkey[i] = reverse_bit(roundkey[i]);
    //     printf("%x\n", roundkey[i]);
    // }
}
