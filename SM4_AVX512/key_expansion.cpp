...
inline uint32_t key_T(uint32_t x) {
    ...
    x = x ^ (x << 13 | x >> 19) ^ (x << 23 | x >> 9);
    return x;
}

inline uint32_t next(const uint32_t *x, const uint32_t *y) {
    return x[0] ^ key_T(x[1] ^ x[2] ^ x[3] ^ *y);
}
...
void key_expansion() {
    ...
    for (int i = 4; i < 36; ++i)
        rk[i] = next(&rk[i - 4], &CK[i - 4]);

    for (int i = 0; i < 32; ++i) {
        roundkey[i] = reverse_bit(rk[i + 4]);
    }
}
