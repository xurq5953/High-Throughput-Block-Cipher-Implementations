...
inline void reverse8(uint8_t &a) {
    uint8_t rvmask[3]{0x55, 0x33, 0x0f};
    for (int i = 0; i < 3; ++i) {
        a = ((a & rvmask[i]) << (1 << i)) | ((a & ~rvmask[i]) >> (1 << i));
    }
}

inline __m256i mm256_not_si256(const __m256i &a) {
    return _mm256_xor_si256(a, _mm256_cmpeq_epi32(a, a));
}

static inline __m128i SHL128(const  __m128i &v, int n)
{ 
    __m128i v1, v2; 
...
static inline __m128i SHR128(const __m128i &v, int n)
{ 
    __m128i v1, v2; 
...
//    std::cout << std::endl << Mbits << "M bits data" << std::endl;
